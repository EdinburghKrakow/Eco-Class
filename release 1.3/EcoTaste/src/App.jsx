...
import {
  fetchGoods, fetchDescription, registerUser, loginUser, getUserProfile, updateUserProfile,
  getUserCart, addToCart, updateCartItem, checkoutOrder, getOrderHistory,
  createGood, updateGood, deleteGood, getAdminOrders, updateOrderStatus, getSalesAnalytics, clearCart, getUserAddresses,
  addUserAddress, updateUserAddress, deleteUserAddress, getSellerAnalytics, selectUserAddress, resetPassword,
} from './api';
...
function App() {
  ...
  const [isSellerAnalyticsLoading, setIsSellerAnalyticsLoading] = useState(false);
  ...
  const loadGoods = async () => {
    try {
      ...
      console.log('Сервер вернул goods:', data);
      setGoods(data);
      setNovelties(data.filter(good => good.isnew));
      setRecommendations(data);
    } catch (error) {
      console.error('Ошибка в loadGoods:', error);
  ...
  // ===== СОХРАНЕНИЕ ТЕКУЩЕГО ЭКРАНА ДЛЯ ОБЫЧНОГО ПОЛЬЗОВАТЕЛЯ =====
  useEffect(() => {
    if (isAdminPanel || !currentUser?.login) return;
    const screenState = {
      showProfile,
      showCart,
      showHistory,
    ...
    localStorage.setItem('ecotaste_last_screen', JSON.stringify(screenState));
  }, [showProfile, showCart, showHistory, showLogin, showRegister, showMenuButtons, isAdminPanel, currentUser]);
    ...
    const restoreSession = async () => {
      try {
        ...
        if (profile.isAdmin === true) {
          ...
          // ✅ Восстановление сохранённого экрана для обычного пользователя
          const savedScreen = localStorage.getItem('ecotaste_last_screen');
          if (savedScreen) {
            try {
              const screen = JSON.parse(savedScreen);
              if (screen.showProfile) setShowProfile(true);
              if (screen.showCart) setShowCart(true);
              if (screen.showHistory) setShowHistory(true);
              if (screen.showLogin) setShowLogin(true);
              if (screen.showRegister) setShowRegister(true);
              if (screen.showMenuButtons !== undefined) setShowMenuButtons(screen.showMenuButtons);
              localStorage.removeItem('ecotaste_last_screen');
            } catch (e) {
              console.error('Ошибка восстановления экрана:', e);
            }
          }
        }
        ...
        try {
          const addresses = await getUserAddresses(profile.login);
          ...
          console.error('Ошибка загрузки адресов:', addrErr);
        }
      } catch (error) {
  ...
  // Автоматическая загрузка истории, если экран истории активен
  useEffect(() => {
    if (showHistory && currentUser?.login && !isAdminPanel) {
      loadHistoryData(currentUser.login);
    }
  }, [showHistory, currentUser, isAdminPanel]);
  ...
  useEffect(() => {
    if (isAdminPanel && currentUser?.login && adminSection === 'analytics') {
      loadSellerAnalytics();
    }
  }, [isAdminPanel, currentUser, adminSection]);
  ...
  const validateAdminForm = (formData = adminForm) => {
    const trimmedId = String(formData.id).trim();
    const trimmedName = String(formData.name).trim();
    const trimmedPrice = String(formData.price).trim();
    const trimmedQuantity = String(formData.quantity).trim();
  ...
  const handleRemoveAllFromCart = async (good, quantity) => {
    if (!currentUser?.login) {
      alert('Для изменения корзины нужно войти в аккаунт.');
      return;
    }
  ...
  const handleClearCart = async () => {
    if (!currentUser?.login) {
      alert('Для очистки корзины нужно войти в аккаунт.');
      return;
    }
    try {
      await clearCart(currentUser.login);
      setCartItems({});
    } catch (error) {
  ...
  const handleResetSubmit = async () => {
    ...
    if (!trimmedLogin || !trimmedPassword) {
      setResetError('Заполните все поля.');
      return;
    }
    ...
    try {
      ...
      setResetSuccess(result.message || 'Пароль успешно изменён.');
      setTimeout(() => {
        handleBackToLogin();
      }, 3000);
    } catch (error) {
      setResetError(error.message || 'Ошибка сброса пароля.');
    } finally {
  ...
  const loadUserAddresses = async (login) => {
    try {
      const addrs = await getUserAddresses(login);
  ...
  const addr = userAddresses.find(a => a.address_id === addressId);
  if (addr && currentUser?.login) {
    try {
      await selectUserAddress({ login: currentUser.login, address: addr.full_address });
      // Обновляем локальное отображение (не обязательно, но для консистентности)
      const parsed = parseAddressDelivery(addr.full_address);
      ...
      alert('Не удалось установить адрес доставки.');
    }
  }
};
  ...
  const handleAddAddress = async () => {
    ...
    // Формируем полный адрес из полей формы
    const full = [deliveryAddress, house, floor, apartment, intercom].join(';');
    try {
      ...
      await loadUserAddresses(currentUser.login);
      // Очищаем поля формы
      setDeliveryAddress('');
      setHouse('');
      setFloor('');
  ...
  const handleEditAddress = async () => {
    if (!editingAddressId || !currentUser?.login) return;
    const full = [deliveryAddress, house, floor, apartment, intercom].join(';');
    try {
      ...
      alert(error.message || 'Не удалось обновить адрес');
    }
  };
  ...
  const handleDeleteAddress = async (addressId) => {
    if (!currentUser?.login) return;
    try {
      await deleteUserAddress(addressId, currentUser.login);
      await loadUserAddresses(currentUser.login);
      if (selectedAddressId === addressId) setSelectedAddressId(null);
    } catch (error) {
      alert(error.message || 'Не удалось удалить адрес');
    }
  };
  ...
  const startEditAddress = (addressId) => {
    ...
    setDeliveryAddress(parts[0] || '');
    setHouse(parts[1] || '');
    setFloor(parts[2] || '');
    setApartment(parts[3] || '');
    setIntercom(parts[4] || '');
  ...
  // ✅ ИСПРАВЛЕНО: добавлена передача category
  const handleCreateGood = async () => {
    ...
    try {
      ...
      const result = await createGood({
        ...
        category: adminForm.category || '',   // ← добавлено
      });
  ...
  const handleFindGoodForEdit = () => {
    ...
    setAdminForm({
      ...
      category: good.category || '',   // ← подгружаем категорию
    });
  };

  // ✅ ИСПРАВЛЕНО: добавлена передача category из data
  const handleUpdateGood = async (goodData = null) => {
    setAdminError('');
    setAdminSuccess('');
    const data = goodData || adminForm;
    ...
    try {
      ...
      const result = await updateGood({
        ...
        category: data.category || '',   // ← добавлено
      });
  ...
  const handleDeleteGood = async (id = null) => {
    setAdminError('');
    setAdminSuccess('');
    const trimmedId = id ? String(id).trim() : deleteId.trim();
    if (!trimmedId) {
  ...
  const loadSellerAnalytics = async () => {
    if (!currentUser?.login || currentUser.isAdmin !== true) return;
    try {
      setIsSellerAnalyticsLoading(true);
      const data = await getSellerAnalytics();
      console.log('Данные продавца загружены:', data);
      setSellerAnalytics(data);
    } catch (error) {
      console.error('Ошибка загрузки аналитики продавца:', error);
    } finally {
      setIsSellerAnalyticsLoading(false);
    }
  };
  ...
  const handleAdminStatusChange = async (orderNum, nextStatus) => {
    try {
      ...
      if (nextStatus === 'Завершен' || nextStatus === 'Отменен') {
        setAdminOrders((prev) =>
          prev.filter((item) => String(item.order_num) !== String(orderNum))
        );
      } else {
        setAdminOrders((prev) =>
          prev.map((item) =>
  ...
  return (
    <ShopPage
      ...
      handleRemoveAllFromCart={handleClearCart}
      ...
      onStartAddAddress={startAddNewAddress}
      onStartEditAddress={startEditAddress}
      onForgotPassword={handleForgotPassword}
      showForgotPassword={showForgotPassword}
      resetLogin={resetLogin}
      setResetLogin={setResetLogin}
