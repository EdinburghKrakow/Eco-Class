function AdminAnalyticsSection({
  ...
  const filteredTopProducts = isAdmin
    ? salesAnalytics?.topProducts || []
    : (salesAnalytics?.topProducts || []).filter(
        (item) => item.seller === currentUser?.user_name
      );

  const filteredRevenue = filteredTopProducts.reduce(
        ...
        <div className="analytics-section">
          <div className="analytics-summary-grid">
            <div className="analytics-card">
              ...
              <p>{formatPrice(isAdmin ? salesAnalytics.totalRevenue : filteredRevenue)} ₽</p>
            </div>
	    {isAdmin && (
	      <>
                <div className="analytics-card">
          ...
          <table className="admin-goods-table">
            ...
            <tbody>
              {salesAnalytics.topProducts && salesAnalytics.topProducts.length > 0 ? (
                filteredTopProducts.slice(0, 10).map((item, index) => (
