function AdminOrdersSection({
  ...
  return (
    <>
      ...
      {(!isAdminOrdersLoading && !adminOrdersError && (currentUser?.user_name !== 'Admin' || adminOrders.length === 0)) && (
        <div className="admin-placeholder-box">Нет активных заказов</div>
      )}

      {!isAdminOrdersLoading && currentUser?.user_name === 'Admin' && adminOrders.length > 0 && (
