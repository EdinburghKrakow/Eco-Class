...
function ShopPage({
  ...
  const isAuthScreen = showProfile || showRegister || showLogin || showForgotPassword;
  const categoriesRef = useRef(null);

  const scrollCategories = (direction) => {
    if (!categoriesRef.current) return;
    if (direction === 'next') {
      categoriesRef.current.scrollBy({ left: 270, behavior: 'smooth' });
    } else {
      categoriesRef.current.scrollBy({ left: -270, behavior: 'smooth' });
    }
  };
  ...
  const categories = [
    { img: '/src/etc_img/vegetables.png', label: 'Овощи и зелень' },
    { img: '/src/etc_img/fruits.png', label: 'Фрукты и ягоды' },
    { img: '/src/etc_img/dairy.png', label: 'Молочные продукты' },
    { img: '/src/etc_img/meat.png', label: 'Мясо и птица' },
    { img: '/src/etc_img/eggs.png', label: 'Яйца' },
    { img: '/src/etc_img/groceries.png', label: 'Бакалея' },
  ];

  const renderCategories = () => (
    <div className="categories-section">
      <button className="category-slider-btn prev" onClick={() => scrollCategories('prev')} aria-label="Предыдущие категории">
        ←
      </button>
      <div className="categories-grid" ref={categoriesRef}>
        {categories.map(({ img, label }) => (
          <div
            key={label}
            className={`product-card ${category === label ? 'active' : ''}`}
      ...
      <button className="category-slider-btn next" onClick={() => scrollCategories('next')} aria-label="Следующие категории">
        →
      </button>
    </div>
  );
  ...
  console.log('ShopPage novelties:', novelties);
  console.log('ShopPage recommendations:', recommendations);
  ...
  return (
    <div className="App">
      ...
      {!isAuthScreen && !showHistory && !showCart && (
        <div className="home-banners-container">
          <img src="/src/etc_img/under_home.png" alt="Баннер под главным" className="under-home-banner" />
        </div>
      )}
        ...
        <div className="auth-screen-container">
          {showProfile && (
            <ProfileForm
              ...
              onStartAddAddress={onStartAddAddress}
        ...
        <div className="content">
          ...
          {!showHistory && !showCart && (
            <>
              ...
              />
              <div className="home-banners-container">
                <img src="/src/etc_img/home_banner.png" alt="Баннер под главным" className="under-home-banner" />
              </div>
              <ProductSection
      ...
      {modalData && (
        <ProductModal
          ...
          onRemoveAll={() => handleRemoveAllFromCart(modalData, modalCartQuantity)}
        />
      )}
    </div>
  );
