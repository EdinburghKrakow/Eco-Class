...
// --- Админские операции с товарами (теперь с JWT) ---
export const createGood = async ({ id, name, price, quantity, isnew, category }) => {
  try {
    const response = await authFetch(`${API_BASE}/goods`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, name, price, quantity, isnew, category }),
    });
...
export const updateGood = async ({ id, name, price, quantity, isnew, category }) => {
  try {
    const response = await authFetch(`${API_BASE}/goods/${encodeURIComponent(id)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ name, price, quantity, isnew, category }),
    });
    const data = await response.json();
...
export const clearCart = async (login) => {
  try {
    const response = await authFetch(`${API_BASE}/cart/cart/clear/${encodeURIComponent(login)}`, {
      method: 'DELETE',
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка очистки корзины');
    return data;
  } catch (error) {
...
export const getUserAddresses = async (login) => {
  try {
    const response = await authFetch(`${API_BASE}/auth/addresses/${encodeURIComponent(login)}`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка загрузки адресов');
    return data;
  } catch (error) {
...
export const addUserAddress = async ({ login, full_address, label }) => {
  try {
    const response = await authFetch(`${API_BASE}/auth/addresses`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ login, full_address, label }),
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка добавления адреса');
    return data;
  } catch (error) {
...
export const updateUserAddress = async (addressId, { login, full_address, label }) => {
  try {
    const response = await authFetch(`${API_BASE}/auth/addresses/${encodeURIComponent(addressId)}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ login, full_address, label }),
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка обновления адреса');
    return data;
  } catch (error) {
...
export const deleteUserAddress = async (addressId, login) => {
  try {
    const response = await authFetch(`${API_BASE}/auth/addresses/${encodeURIComponent(addressId)}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка удаления адреса');
    return data;
  } catch (error) {
...
export const getSellerAnalytics = async () => {
  try {
    const response = await authFetch(`${API_BASE}/orders/seller/analytics`);
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка загрузки аналитики продавца');
    return data;
  } catch (error) {
...
export const selectUserAddress = async ({ login, address }) => {
  try {
    const response = await authFetch(`${API_BASE}/auth/select-address`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ login, address }),
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'Ошибка выбора адреса');
    return data;
  } catch (error) {
...
export const resetPassword = async ({ login, newPassword }) => {
  try {
