...
export const selectAddress = async (req, res) => {
  try {
    ...
    if (!login || !address) {
      return res.status(400).json({ error: 'Не указан логин или адрес.' });
    }

    const result = await authService.updateUserProfile(login, {
    ...
    res.status(200).json({ message: 'Адрес доставки обновлён.' });
  } catch (err) {
    console.error('Ошибка выбора адреса:', err);
    res.status(500).json({ error: 'Ошибка сервера при выборе адреса.' });
  }
};
