...
export const getCartByLogin = async (login) => {
  ...
  if (result.rows.length === 0) {
    return { exists: false, login, goods_cart: '', quantity: '', items: {} };
  }
...
export const clearCartByLogin = async (login) => {
  await pool.query('UPDATE user_cart SET goods_cart = NULL, quantity = NULL WHERE login = $1', [login]);
};
